...
RC deQueue() 
{
	...
	for (int i = 0; i < (*queue).totalNumOfFrames; i++)
	{
        ...
        {
            //((*pinfo).pageNum == (*queue).tail->pageNum) ? 
            //(pageDelete=(*pinfo).pageNum), ((*queue).tail = ((*queue).tail->prevPageInfo)), ((*queue).tail->nextPageInfo = NULL) : 
            //(pageDelete=(*pinfo).pageNum), ((*pinfo).prevPageInfo->nextPageInfo = (*pinfo).nextPageInfo), (*pinfo).nextPageInfo->prevPageInfo = (*pinfo).prevPageInfo;
